...
$display = "<table class='table table-hover text-nowrap' id='pendingcancellation_datatable' style='font-size: 12px;'>
        ...
        $sql = "SELECT  joborderidz, joborderidxx, IFNULL(vbrand, '-') as vbrand, IFNULL(vmodel, '-') as vmodel, 
                        ...
                        CASE
                            WHEN approvaldate IS NOT NULL THEN CONCAT(DATEDIFF(CURDATE(), approvaldate), ' DAYS')
                            WHEN approvaldate IS NULL THEN CONCAT(DATEDIFF(CURDATE(), DATE(cancellationrequested)), ' DAYS')
                ...
                WHERE approvalstatus = 3 AND joborderidxx IS NOT NULL AND 
                CASE 
